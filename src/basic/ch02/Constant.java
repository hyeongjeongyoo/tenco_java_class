 ...
 * 런타임 시점 --> ctrl + F11 프로그램을 실행 ~~ 실행 종료까지
 * 
 * java 명령어는 프로그램을 실행 시키는 명령어이다.
 * javac 명령어는 번역하는 과정이다.
 * 
 * 자바 언어로 코드를 작성하고 있다.
 * JDK 도구를 설치해야 자바 프로그래밍 가능 -> 여러 도구들이 존재
 * JDK -> JRE(자바 프로그램 실행 환경이라고 한다.)
 * JRE --> JVM(자바 가상 머신)
 * JVM은 .class 파일을 실행시켜 주는 녀석이다.
 * JVM은 어떤 연산을 거쳐 이진코드(바이너리 파일로 변환 시켜서 수행하는 녀석이다.)
 */
public class Constant {
	...
	public static void main(String[] args) {
		...
		//  상수 선언 -> 값이 한 번 정해지면 변경 할 수 없는 수(값, 데이터)
		final int MAX_NUM = 10;
		final int MIN_NUM;
		MIN_NUM = 0;
		
		//MAX_NUM = 100; // 컴파일 오류 발생
		// ctrl + s 누르면 IDE 툴이 java Constant.java 명령어를 수행한다.
