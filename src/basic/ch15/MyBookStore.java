...
public class MyBookStore {
	...
	public static void save(Scanner sc, Book[] books) {
		...
		// [1] <--
		// [2] <--
		// [3] <-- null 이라면 저장 Book book = new Book(bookTitle, bookAuthor);
		for (int i = 0; i < books.length; i++) {
			if (books[i] == null) {
				books[i] = book;
	...
	public static void readByTitle(Scanner sc, Book[] books) {
		...
		// 문자열비교 -> equals
		// books[0].getTitle().equals() // 배열 안에 객체가 없다면 null pointer -> 방어적 코드

		// 만약 사용자가 입력한 책 제목과 배열 요소안에 title 값이 같다면
		// 화면에 책 제목, 책 저자 이름을 출력하고
		// 아니라면 해당하는 책이 없습니다. 문구 출력하시오.
